...
@Service
public class MpesaBalanceInquiryService {
    ...
    @Value("${MpesaApis.balance.inquiry.url}")
    ...
    public String inquireBalance(String accessToken, String remarks) {
        try {
            RestTemplate restTemplate = new RestTemplate();
            HttpHeaders headers = new HttpHeaders();
            ...
            System.out.println("Balance Inquiry Request Payload: " + requestBody);
            System.out.println("Authorization Header: Bearer " + accessToken);

            HttpEntity<String> entity = new HttpEntity<>(requestBody, headers);

            ResponseEntity<String> response = restTemplate.exchange(balanceInquiryUrl, HttpMethod.POST, entity, String.class);
            ...
            if (response.getStatusCode() == HttpStatus.OK) {
                return parseBalanceInquiryResponse(response.getBody());
            } else {
                throw new RuntimeException("Failed to inquire balance: " + response.getStatusCode() + " - " + response.getBody());
            }
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException("Failed to inquire balance: " + e.getMessage(), e);
        }
    }

    private String parseBalanceInquiryResponse(String responseBody) {
        try {
            ObjectMapper mapper = new ObjectMapper();
            ...
            JsonNode resultParametersNode = resultNode.path("ResultParameter").path("ResultParameters");
            ...
            if (resultParametersNode.isArray()) {
                for (JsonNode parameterNode : resultParametersNode) {
                    ...
                    resultParameters.append(key).append(": ").append(value).append("\n");
                }
            }

            return String.format(
                    "ResultType: %s\nResultCode: %s\nResultDesc: %s\nOriginatorConversationID: %s\nConversationID: %s\nTransactionID: %s\n%s",
                    resultType, resultCode, resultDesc, originatorConversationID, conversationID, transactionID, resultParameters.toString()
            );
        } catch (Exception e) {
            throw new RuntimeException("Failed to parse balance inquiry response: " + e.getMessage(), e);
        }
    }
